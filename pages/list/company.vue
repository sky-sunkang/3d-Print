...
<script>
  ...
  export default {
    ...
    head(){
      return {
        title: '深圳汇通三维打印科技_中山3D打印_中山手板模型_石岐3D打印公司|中山东区|中山南区|中山西区|港口|南朗|南头|深圳|东莞|惠州|广州|佛山|珠海|3D打印',
        meta: [
          { hid: 'keywords', name: 'keywords', content: '中山3D打印_中山3D打印公司_石岐3D打印_东区3D打印_南区3D打印_西区3D打印_中山五桂山3D打印_小榄3D打印_横栏3D打印_东升3D打印_深圳|东莞|惠州|广州|佛山|汕头|3D打印' },
    ...
    async asyncData({ $axios}) {
      //产品
      const res = await $axios.get('/member/content.json?id=9')
...
//      var aa = $('<p/>').html(res.data.data[0].text).text()
      ...
      console.log(prores)
      return {company:res.data,leftproducts:prores.data.page.list}
    }
  }
</script>
